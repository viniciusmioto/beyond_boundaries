...
OUTPUT_FILE = "../../data/processed/3_authors.csv"
...
# Create rows for each author - authors are already uniquely identified by author_id in the dictionary
...
# Check for duplicate IDs (this should never happen with the current implementation,
# but adding as an explicit verification)
duplicate_ids = df_authors[df_authors.duplicated(subset=['id'])]
if not duplicate_ids.empty:
    print(f"Warning: Found {len(duplicate_ids)} duplicate author IDs")
    # Keep only the first occurrence of each ID
    df_authors = df_authors.drop_duplicates(subset=['id'], keep='first')
...
print("Authors CSV created successfully with duplicate ID handling!")